...
class Video_recording_control(QMainWindow):
    ...
    def __init__(self):
        ...
        for rb in sorted([RASPBERRY_MAC_ADDR[x] for x in RASPBERRY_MAC_ADDR]):
            ...
            combo.currentIndexChanged[int].connect(
                partial(self.combo_index_changed, rb)
            )
            ...
            self.image_label[rb].setSizePolicy(
            ...
            self.status_list[rb] = QPushButton(
                "Status", clicked=partial(self.status_one, rb, output=True)
            )
            ...
            l2.addWidget(
                QPushButton("Clear output", clicked=partial(self.clear_output, rb))
            )
            l2.addWidget(QPushButton("Blink", clicked=partial(self.blink, rb)))
            l2.addWidget(
                QPushButton("Send key", clicked=partial(self.send_public_key, rb))
            )
            ...
            right_layout.addWidget(
                QPushButton("Take one picture", clicked=partial(self.one_picture, rb))
            )

            right_layout.addWidget(QLabel("<b>Video</b>"))
            l2 = QHBoxLayout()
            self.video_streaming_btn[rb] = QPushButton(
                "Start video streaming",
                clicked=partial(self.video_streaming, rb, "start"),
            )
            l2.addWidget(self.video_streaming_btn[rb])
            l2.addWidget(
                QPushButton(
                    "Stop video streaming",
                    clicked=partial(self.video_streaming, rb, "stop"),
                )
            )
            self.record_button[rb] = QPushButton(
                "Start video recording", clicked=partial(self.start_video_recording, rb)
            )
            l2.addWidget(self.record_button[rb])
            l2.addWidget(
                QPushButton(
                    "Stop video recording",
                    clicked=partial(self.stop_video_recording, rb),
                )
            )
            right_layout.addLayout(l2)
            ...
            self.download_button[rb] = QPushButton(
                "Download all video", clicked=partial(self.download_all_video, rb, "")
            )
            l2.addWidget(self.download_button[rb])

            l2.addWidget(
                QPushButton(
                    "Delete all video", clicked=partial(self.delete_all_video, rb)
                )
            )
            right_layout.addLayout(l2)
            ...
            l2 = QHBoxLayout()
            l2.addWidget(
                QPushButton("Send command", clicked=partial(self.send_command, rb))
            )
            ...
            if rasp_count % GUI_COLUMNS_NUMBER == 0 or rasp_count == len(
                RASPBERRY_MAC_ADDR
            ):
        ...
        hlayout_all_buttons.addWidget(
            QPushButton(
                "Status from all", clicked=partial(self.status_all, output=True)
            )
        )
        hlayout_all_buttons.addWidget(
            QPushButton("Sync time all", clicked=self.sync_all)
        )
        hlayout_all_buttons.addWidget(
            QPushButton("video list from all", clicked=self.video_list_from_all)
        )
        hlayout_all_buttons.addWidget(
            QPushButton(
                "Download video from all", clicked=self.download_all_video_from_all
            )
        )
        # hlayout_all_buttons.addWidget(QPushButton("Update all server", clicked=self.update_all))
        hlayout_all_buttons.addWidget(
            QPushButton("Send command to all", clicked=self.send_command_all)
        )
        hlayout_all_buttons.addWidget(
            QPushButton("Clear all output", clicked=self.clear_all_output)
        )
        hlayout_all_buttons.addWidget(
            QPushButton("Reboot all", clicked=self.reboot_all)
        )
        hlayout_all_buttons.addWidget(
            QPushButton("Shutdown all", clicked=self.shutdown_all)
        )
        hlayout_all_buttons.addWidget(
            QPushButton("Scan network", clicked=partial(self.scan_network, output=True))
        )
        ...
        hlayout_navigation_buttons.addItem(
                ...
                try:
                    ...
                    if mac_addr in RASPBERRY_MAC_ADDR:
                        ...
                        if remote_hostname != RASPBERRY_MAC_ADDR[mac_addr]:
                            r2 = requests.get(
                                f"http://{ip_addr}:{SERVER_PORT}/set_hostname/{RASPBERRY_MAC_ADDR[mac_addr]}"
                            )
                            if r2.status_code == 200:
                                r2_dict = eval(r2.text)
                                logging.info(
                                    f"{ip_addr}: hostname change success: {r2_dict.get('new_hostname', '')}"
                                )
                            else:
                                logging.info(
                                    f"{ip_addr}: hostname change failed: {RASPBERRY_MAC_ADDR[mac_addr]}"
                                )
                        # set raspberry time date
                        date, hour = date_iso().split(" ")
                        try:
                            r3 = requests.get(
                                f"http://{ip_addr}:{SERVER_PORT}/sync_time/{date}/{hour}"
                            )
    ...
    def scan_raspberries(self):
        ...
        ip_list = [
            f"{ip_mask}.{x}" for x in range(WLAN_INTERVAL[0], WLAN_INTERVAL[1] + 1)
        ]
        self.RASPBERRY_IP = {}
        threads = []
        ...
        for x in threads:
            x.join()
        logging.info(
            f"testing subnet done: found {len(self.RASPBERRY_IP)} clients: {self.RASPBERRY_IP}"
        )
    ...
    def scan_network(self, output):
        ...
        self.message_box.setText(
            f"Scanning done: {len(self.RASPBERRY_IP)} client(s) found"
        )
    ...
    def video_streaming(self, rb, action):
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            if action == "start":
                ...
                r = requests.get(
                    f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/video_streaming/start?&w={w}&h={h}"
                )
                if r.status_code == 200:
                    ...
                    return
                time.sleep(1)
                self.media_list[rb].setMedia(
                    QMediaContent(
                        QUrl(f"http://{self.RASPBERRY_IP[rb]}:9090/stream/video.mjpeg")
                    )
                )
                ...
                try:
                    ...
                    img = qrcode.make(
                        f"http://{self.RASPBERRY_IP[rb]}:9090/stream/video.mjpeg"
                    )
                    self.image_label[rb].setPixmap(
                        QPixmap.fromImage(ImageQt(img))
                    )  # .scaled(self.image_label[rb].size(), Qt.KeepAspectRatio))
                except:
                    logging.INFO("qrcode module not installed")

            if action == "stop":
                r = requests.get(
                    f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/video_streaming/stop"
                )
                if r.status_code == 200:
    ...
    def send_public_key(self, rb):
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            if (
                pathlib.Path.home() / pathlib.Path(".ssh") / pathlib.Path("id_rsa.pub")
            ).is_file():
                # read content of id_rsa.pub file
                with open(
                    pathlib.Path.home()
                    / pathlib.Path(".ssh")
                    / pathlib.Path("id_rsa.pub"),
                    "r",
                ) as f_in:
                ...
                r = requests.get(
                    f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/add_key/{base64.b64encode(file_content.encode('utf-8')).decode('utf-8')}"
                )
                if r.status_code == 200:
    ...
    def video_list(self, rb):
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            try:
                r = requests.get(
                    f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/video_list"
                )
                if r.status_code == 200:
    ...
    def send_command(self, rb):
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            ...
            try:
                ...
                r = requests.get(
                    f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/command/{cmd}"
                )
                if r.status_code == 200:
                    r_dict = eval(r.text)
                    if r_dict.get("status", "") != "error":
                        self.rb_msg(
                            rb, f"Return code: {r_dict.get('return_code', '-')}"
                        )
    ...
    def reboot(self, rb):
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            text, ok = QInputDialog.getText(
                self, "Reboot rasberry", "confirm writing 'yes'"
            )
    ...
    def reboot_all(self):
        ...
        """
        text, ok = QInputDialog.getText(
            self, "Reboot all rasberries", "confirm writing 'yes'"
        )
        if not ok or text != "yes":
            return
        ...
        for rb in sorted([RASPBERRY_MAC_ADDR[x] for x in RASPBERRY_MAC_ADDR]):
            if self.raspberry_status[rb]:
                try:
                    r = requests.get(
                        f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/reboot"
                    )
    ...
    def shutdown(self, rb):
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            text, ok = QInputDialog.getText(
                self, "Shutdown rasberry", "confirm writing 'yes'"
            )
            if not ok or text != "yes":
                return
            try:
                r = requests.get(
                    f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/shutdown"
                )
                if r.status_code == 200:
                    self.rb_msg(rb, r.text)
                else:
    ...
    def shutdown_all(self):
        ...
        """
        text, ok = QInputDialog.getText(
            self, "Shutdown all rasberries", "confirm writing 'yes'"
        )
        if not ok or text != "yes":
            return
    ...
    def blink(self, rb):
        ...
        if self.RASPBERRY_IP.get(rb, ""):
            ...
            try:
                r = requests.get(
                    f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/blink", timeout=2
                )
            except Exception:
                self.rb_msg(rb, "Error")
                self.status_one(rb)
        ...
        try:
            if self.RASPBERRY_IP.get(rb, ""):
                ...
                if not r1:
                    ...
                    r = requests.get(
                        f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/status",
                        timeout=TIME_OUT,
                    )
                    if r.status_code == 200:
                        r_dict = eval(r.text)
    ...
    def status_one(self, rb, output=True):
        ...
        self.status2(rb)
        if output:
            self.rb_msg(rb, json_formater(str(self.status_dict[rb])))
    ...
    def update_raspberry_display(self, rb):
        ...
        self.tw.setTabIcon(
            sorted(RASPBERRY_MAC_ADDR.values()).index(rb), QIcon(f"{color}.png")
        )
    ...
    def sync_time(self, rb):
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            ...
            try:
                r = requests.get(
                    f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/sync_time/{date}/{hour}"
                )
                self.rb_msg(rb, json_formater(r.text))
            except Exception:
                self.rb_msg(rb, "Error")
    ...
    def one_picture(self, rb):
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            try:
                ...
                r = requests.get(
                    f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/one_picture?w={w}&h={h}"
                )

                r2 = eval(r.text)
                if r2["result"] == "OK":
                    r = requests.get(
                        f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/static/live.jpg",
                        stream=True,
                    )
                    if r.status_code == 200:
                        ...
                        self.image_label[rb].setPixmap(
                            QPixmap(f"live_{rb}.jpg").scaled(
                                self.image_label[rb].size(), Qt.KeepAspectRatio
                            )
                        )
    ...
    def start_video_recording(self, rb):
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            ...
            try:
                r = requests.get(
                    (
                        f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/start_video?"
    ...
    def stop_video_recording(self, rb):
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            try:
                ...
                r = requests.get(
                    f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/stop_video"
                )
                self.rb_msg(rb, r.text)
                self.status_one(rb, output=False)
            except Exception:
    ...
    def delete_all_video(self, rb):
        ...
        """
        text, ok = QInputDialog.getText(
            self, f"Delete all video from Rasberry {rb}", "confirm writing 'yes'"
        )
        if not ok or text != "yes":
            return
        try:
            ...
            r = requests.get(
                f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/delete_all_video"
            )
            if r.status_code == 200:
                r2 = eval(r.text)
                if r2.get("status", "") == "OK":
    ...
    def get_log(self, rb):
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            try:
                r = requests.get(
                    f"http://{self.RASPBERRY_IP[rb]}:{SERVER_PORT}/get_log"
                )
                self.rb_msg(rb, r.text)
            except Exception:
                self.rb_msg(rb, "Error")
    ...
    def update_all(self):
        ...
        """
        text, ok = QInputDialog.getText(
            self, "Update server on all raspberries", "confirm writing 'yes'"
        )
        if not ok or text != "yes":
            return
        ...
        for rb in sorted([RASPBERRY_MAC_ADDR[x] for x in RASPBERRY_MAC_ADDR]):
            if self.raspberry_status[rb]:
                ...
                r = os.system(
                    f"scp server.py pi@{self.RASPBERRY_IP[rb]}:{CLIENT_PROJECT_DIRECTORY}"
                )
                if not r:
    ...
    def download_all_video(self, rb, download_dir=""):
        ...
        """
        download_dir = "/home/user/rana2021/video"
        if download_dir == "":
        ...
        if rb in self.RASPBERRY_IP and self.raspberry_status[rb]:
            ...
            self.download_process[rb].readyReadStandardOutput.connect(
                lambda: self.read_process_stdout(rb)
            )
            self.download_process[rb].error.connect(lambda x: self.process_error(x, rb))
            self.download_process[rb].finished.connect(
                lambda exitcode: self.download_finished(exitcode, rb)
            )

            self.download_process[rb].start(
                "rsync",
                [
                    "-avz",
    ...
    def download_all_video_from_all(self):
        ...
        """
        text, ok = QInputDialog.getText(
            self, "Download video from all raspberries", "confirm writing 'yes'"
        )
        if not ok or text != "yes":
            return
